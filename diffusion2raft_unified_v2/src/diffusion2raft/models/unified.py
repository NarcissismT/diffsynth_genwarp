...
class QwenEditFeatureEncoder(nn.Module):
    ...
    def _capture_one(
        ...
        *,
        ...
        if set(captured) != set(self.layer_indices):
            missing = sorted(set(self.layer_indices) - set(captured))
            raise RuntimeError(f"failed to capture Qwen feature layers {missing}")
        ...
        for projection, index in zip(self.projections, self.layer_indices, strict=True):
            ...
            if hidden.ndim != 3 or hidden.shape[0] != 1:
                raise RuntimeError(f"unexpected Qwen hidden shape {tuple(hidden.shape)}")
            if hidden.shape[1] <= target_count:
                raise RuntimeError(
                    "Qwen hidden state has no source-condition tokens: "
                    f"tokens={hidden.shape[1]}, target={target_count}"
                )
            target_tokens = hidden[:, :target_count]
            source_tokens = hidden[:, target_count:]
            source_h, source_w = _factor_grid(
                int(source_tokens.shape[1]), width / max(height, 1)
            )
            target_map = target_tokens.transpose(1, 2).reshape(
                1, hidden.shape[-1], target_h, target_w
            )
            source_map = source_tokens.transpose(1, 2).reshape(
                1, hidden.shape[-1], source_h, source_w
            )
...
class FeatureReliabilityFusion(nn.Module):
    ...
    def __init__(self, qwen_channels: int, cnn_channels: int, match_channels: int) -> None:
        super().__init__()
    ...
    def forward(
        ...
        return reference, source, context, confidence
...
def _local_correlation(reference: Tensor, source_at_flow: Tensor, radius: int) -> Tensor:
    ...
    return (patches * reference.unsqueeze(2)).sum(dim=1) / math.sqrt(max(channels, 1))
...
class RecurrentResidualRefiner(nn.Module):
    ...
    def forward(
        ...
        for _ in range(self.iterations):
            ...
            correlation = _local_correlation(reference, source_at_flow, self.radius)
...
class UnifiedDocumentRectifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.fusion = FeatureReliabilityFusion(
            feature_channels, cnn_channels, feature_channels
        )
    ...
    def forward(
        ...
        *,
        ...
        reference, source, context, confidence = self.fusion(
            qwen_target,
