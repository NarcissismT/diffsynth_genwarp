...
def residual_from_composed_flow(
    base_flow: Tensor,
    composed_flow: Tensor,
    *,
    iterations: int = 6,
) -> tuple[Tensor, Tensor]:
    """Recover the target->intermediate residual implied by a final flow.

    ``compose_backward_flows(B, R)`` evaluates ``R(x) + B(x + R(x))``.  The
    commonly used target ``composed_flow - base_flow`` is therefore only an
    approximation and is biased whenever ``B`` varies spatially.  This helper
    solves the correct fixed-point equation

    ``R(x) = composed_flow(x) - B(x + R(x))``.

    It is intended for constructing a detached training target.  The returned
    consistency error is the per-pixel endpoint error obtained after composing
    the recovered residual back with ``base_flow``; callers can mask rare
    non-converged locations.
    """

    _check_flow(base_flow)
    _check_flow(composed_flow)
    if base_flow.shape != composed_flow.shape:
        raise ValueError(
            "base and composed flows must share shape, got "
            f"{tuple(base_flow.shape)} and {tuple(composed_flow.shape)}"
        )
    if iterations < 1:
        raise ValueError(f"iterations must be >= 1, got {iterations}")

    # Additive subtraction is a useful first estimate.  Fixed-point updates
    # then account for evaluating the spatially varying base at x + R(x).
    residual = composed_flow - base_flow
    for _ in range(int(iterations)):
        ...
        residual = composed_flow - sampled_base

    recomposed = compose_backward_flows(base_flow, residual)
    consistency_error = torch.linalg.vector_norm(
        recomposed - composed_flow,
        dim=1,
        keepdim=True,
    )
    return residual, consistency_error
