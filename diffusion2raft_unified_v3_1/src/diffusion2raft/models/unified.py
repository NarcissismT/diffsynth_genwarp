...
from .teacher_prior import TorchScriptGeometryPrior
...
class QwenEditFeatureEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Multi-GPU sharding: the 20B transformer (~39GB bf16) plus the 16GB
        # text encoder do not fit on a single 24GB card.  When qwen.device_map
        # is set we shard the frozen transformer across GPUs with accelerate,
        # keeping bf16 features identical to training (unlike 4bit).  The rest
        # of the pipeline (VAE, text encoder) is placed on a chosen device.
        device_map = config.get("device_map")
        sharded_transformer = None
        if device_map is not None and quantization == "none":
            if not str(device).startswith("cuda"):
                raise ValueError("qwen.device_map requires a CUDA device")
            from diffusers import QwenImageTransformer2DModel

            n_gpu = torch.cuda.device_count()
            per_gpu = str(config.get("device_map_max_memory_per_gpu", "20GiB"))
            # Reserve the rectifier GPU and the support (text encoder / VAE) GPU
            # so accelerate does not pack transformer layers onto them, which
            # would leave no room for the 16GB text encoder or the trainable
            # rectifier heads.
            rectifier_index = torch.device(device).index or 0
            support_device = str(
                config.get("device_map_support_device", f"cuda:{max(0, n_gpu - 1)}")
            )
            support_index = torch.device(support_device).index or 0
            reserved = {int(rectifier_index), int(support_index)}
            max_memory = {
                index: ("0GiB" if index in reserved else per_gpu)
            ...
            if config.get("revision") is not None:
                transformer_kwargs["revision"] = str(config["revision"])
            sharded_transformer = QwenImageTransformer2DModel.from_pretrained(
                model_id, **transformer_kwargs
            )
            sharded_transformer.requires_grad_(False)
            sharded_transformer.eval()
            load_kwargs["transformer"] = sharded_transformer

        pipeline = pipeline_cls.from_pretrained(model_id, **load_kwargs)
        if sharded_transformer is not None:
            # The transformer is already sharded across GPUs by accelerate.  Move
            # only the remaining light components to the support device (computed
            # above) so that pipeline.to(device) does not try to relocate the
            # sharded module.
            for component_name in ("vae", "text_encoder"):
                component = getattr(pipeline, component_name, None)
                if component is not None:
                    component.to(support_device)
        elif bool(config.get("cpu_offload", False)) and str(device).startswith("cuda"):
    ...
    def _capture_one(
        ...
        *,
        ...
        for projection, index in zip(self.projections, self.layer_indices, strict=True):
            ...
            projection_param = next(projection.parameters())
            # With a sharded transformer the captured hidden states can live on
            # a different GPU than the (trainable) projection heads.  Move both
            # device and dtype so the projection runs where its weights are.
            target_map = target_map.to(
                device=projection_param.device, dtype=projection_param.dtype
            )
            source_map = source_map.to(
                device=projection_param.device, dtype=projection_param.dtype
            )
...
class FeatureReliabilityFusion(nn.Module):
    ...
    Qwen has two distinct jobs here: its source/target similarity can propose a
    correspondence, while its generation tokens can provide semantic context
    to the recurrent update block.  Those jobs must not share the same gate.
    A poor cost volume should fall back to the source-faithful CNN match without
    also deleting useful generation context.

    ``match_confidence_power`` and ``match_confidence_cap`` can conservatively
    calibrate the gate used by the cost-volume features.  The context path uses
    the raw gate with a configurable floor.  Both consumed gates can be detached
    from the refiner loss so the confidence head is calibrated by its explicit
    oracle BCE target rather than being inflated merely because context helps.
    The numerical defaults preserve the legacy forward pass; the safe v3.2
    recipe opts into the stricter power/cap.  These controls add no parameters,
    so legacy state dicts remain strictly loadable.
    """
    ...
    def __init__(
        ...
        *,
        ...
        match_confidence_power: float = 1.0,
        match_confidence_cap: float = 1.0,
        context_confidence_floor: float = 0.0,
        detach_confidence_from_refiner: bool = False,
    ) -> None:
        super().__init__()
        self.shared_qwen_projection = bool(shared_qwen_projection)
        self.match_confidence_power = float(match_confidence_power)
        self.match_confidence_cap = float(match_confidence_cap)
        self.context_confidence_floor = float(context_confidence_floor)
        self.detach_confidence_from_refiner = bool(detach_confidence_from_refiner)
        if self.match_confidence_power < 1.0:
            raise ValueError("match_confidence_power must be >= 1 for safe fusion")
        if not 0.0 <= self.match_confidence_cap <= 1.0:
            raise ValueError("match_confidence_cap must be in [0, 1]")
        if not 0.0 <= self.context_confidence_floor <= 1.0:
            raise ValueError("context_confidence_floor must be in [0, 1]")
    ...
    def forward(
        ...
        *,
        force_matching_fallback: Tensor | None = None,
        force_context_fallback: Tensor | None = None,
    ) -> tuple[
        Tensor,
        Tensor,
        Tensor,
        ...
        availability = torch.ones_like(raw_confidence)
        if force_fallback is not None:
            availability = availability * (
                1.0 - force_fallback.to(raw_confidence.dtype)
            )
        matching_availability = availability
        if force_matching_fallback is not None:
            matching_availability = matching_availability * (
                1.0 - force_matching_fallback.to(raw_confidence.dtype)
            )
        context_availability = availability
        if force_context_fallback is not None:
            context_availability = context_availability * (
                1.0 - force_context_fallback.to(raw_confidence.dtype)
            )

        # Keep the public legacy effective gate for checkpoint diagnostics, but
        # do not use it for both jobs.  The epoch-10 run had raw gate 0.203 vs
        # oracle 0.036; squaring gives 0.041, while the cap bounds damage from
        # a severely over-confident bad correspondence.
        effective_confidence = raw_confidence * torch.minimum(
            matching_availability, context_availability
        )
        refiner_confidence = (
            raw_confidence.detach()
            if self.detach_confidence_from_refiner
            else raw_confidence
        )
        match_confidence = refiner_confidence.pow(self.match_confidence_power)
        match_confidence = match_confidence.clamp(max=self.match_confidence_cap)
        match_confidence = match_confidence * matching_availability
        context_confidence = refiner_confidence.clamp_min(
            self.context_confidence_floor
        )
        context_confidence = context_confidence * context_availability

        # At zero match confidence both sides become the exact same
        # source-faithful CNN feature, giving the correlation decoder an
        # identity/zero-residual fallback.  Qwen can still help through context.
        reference = match_confidence * target + (1.0 - match_confidence) * source_cnn
        source = (
            match_confidence * qwen_enhanced_source
            + (1.0 - match_confidence) * source_cnn
        )
        ...
        context = self.context(
            torch.cat(
                (
                    context_confidence * target,
                    context_confidence * source_qwen,
                    source_cnn,
                    cnn_source,
                    prior_flow_low,
                ),
...
class RecurrentResidualRefiner(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        correlation_temperature = float(correlation_temperature)
        if (
            not math.isfinite(correlation_temperature)
            or correlation_temperature <= 0.0
        ):
            raise ValueError(
                "correlation temperature must be finite and positive, "
                f"got {correlation_temperature}"
            )
    ...
    def set_correlation_temperature(self, temperature: float) -> None:
        ...
        self.correlation_temperature = temperature
...
class UnifiedDocumentRectifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.fusion = FeatureReliabilityFusion(
            ...
            match_confidence_power=match_confidence_power,
            match_confidence_cap=match_confidence_cap,
            context_confidence_floor=context_confidence_floor,
            detach_confidence_from_refiner=detach_confidence_from_refiner,
        )
        ...
        # Ablation switch: when True, the Qwen gate is forced to 0 at every
        # pixel so reference/source collapse to the CNN fallback. Lets a single
        # trained checkpoint be evaluated with vs without the Qwen path, which
        # directly tests whether the diffusion features contribute anything.
        self.qwen_off = False
        # Evaluation-only path switches.  They contain no tensors and do not
        # alter checkpoint schemas.  qwen_off remains the compatibility alias
        # that disables both jobs together.
        self.qwen_matching_off = False
        self.qwen_context_off = False
        # Runtime-only deployment homotopy.  It deliberately is not a buffer or
        # parameter, so adding the teacher warmup does not change legacy v3.1 /
        # v3.2 state_dict schemas.  Learned-prior models keep the historical
        # full-residual behavior unless a caller explicitly changes the scale.
        self._residual_application_scale = 1.0
        self._residual_application_schedule: dict[str, Any] | None = None

    @property
    def prior_backend(self) -> str:
        """Stable backend name stored in checkpoint metadata."""

        return str(getattr(self.prior, "backend_name", "learned"))

    @property
    def teacher_checkpoint_marker_key(self) -> str | None:
        marker_name = getattr(self.prior, "marker_name", None)
        return None if marker_name is None else f"prior.{marker_name}"

    @property
    def teacher_prior_identity(self) -> dict[str, Any] | None:
        """Return the external teacher contract bound to a checkpoint.

        The multi-gigabyte TorchScript module intentionally is absent from the
        unified state dict.  Binding its filesystem identity and preprocessing
        contract prevents a shape-compatible checkpoint from silently running
        against a different external teacher.
        """

        if self.prior_backend != "torchscript":
            return None
        resolved_path = getattr(self.prior, "resolved_checkpoint_path", None)
        file_size = getattr(self.prior, "checkpoint_size_bytes", None)
        mtime_ns = getattr(self.prior, "checkpoint_mtime_ns", None)
        sha256 = getattr(self.prior, "checkpoint_sha256", None)
        if (
            ...
            raise RuntimeError("TorchScript prior does not expose frozen file identity")
        dtype = str(getattr(self.prior, "autocast_dtype", "unknown"))
        identity = {
            "version": 2,
            ...
            "sha256": str(sha256),
            "input_size": int(getattr(self.prior, "input_size")),
            # Historical teachers may accept 518px inputs while still emitting
            # a fixed 512px flow canvas.  Keep these contracts independent.
            "flow_size": int(
                getattr(
                    self.prior,
                    "flow_size",
                    getattr(self.prior, "input_size"),
                )
            ),
            "blur_kernel": int(getattr(self.prior, "blur_kernel")),
            "autocast_dtype": dtype.removeprefix("torch."),
        }
        if bool(getattr(self.prior, "requires_logical_cuda0", False)):
            identity["requires_logical_cuda0"] = True
        return identity

    @property
    def residual_application_scale(self) -> float:
        return float(self._residual_application_scale)

    def set_residual_application_scale(self, scale: float) -> None:
        scale = float(scale)
        if not math.isfinite(scale) or not 0.0 <= scale <= 1.0:
            raise ValueError(
                "residual application scale must be finite and in [0,1], "
                f"got {scale}"
            )
        self._residual_application_scale = scale

    @property
    def checkpoint_compatible_missing_prefixes(self) -> tuple[str, ...]:
        """New identity-initialized branches that may be absent in v3.1.

        Checkpoint loading remains strict for every other key.  The training
        loader also resets optimizer state when this migration is exercised,
        because the new pose parameter group has no saved Adam moments.
        """

        if getattr(self.prior, "global_pose_head", None) is not None:
            return ("prior.global_pose_head.",)
        return ()
    ...
    def forward(
        ...
        *,
        ...
        force_fallback = None
        if self.qwen_off:
            # Qwen-off ablation: force the gate to 0 everywhere -> CNN fallback.
            force_fallback = torch.ones(
                (warped.shape[0], 1, 1, 1), device=warped.device
            ).to(qwen_target.dtype)
        elif self.training and self.feature_dropout_prob > 0:
            force_fallback = (
                torch.rand(
        ...
        force_matching_fallback = (
            torch.ones((warped.shape[0], 1, 1, 1), device=warped.device).to(
                qwen_target.dtype
            )
            if self.qwen_matching_off and not self.qwen_off
            else None
        )
        force_context_fallback = (
            torch.ones((warped.shape[0], 1, 1, 1), device=warped.device).to(
                qwen_target.dtype
            )
            if self.qwen_context_off and not self.qwen_off
            else None
        )
        (
            reference,
            ...
            prior_flow_low,
            force_fallback,
            force_matching_fallback=force_matching_fallback,
            force_context_fallback=force_context_fallback,
        )
        with torch.autocast(device_type=warped.device.type, enabled=False):
            ...
            raw_residuals = self.refiner(
                reference.float(),
                source.float(),
                context.float(),
                warped.shape[-2:],
            )
            residual_scale = self.residual_application_scale
            if residual_scale == 0.0:
                # Do not call grid_sample through compose(B, 0): even an
                # identity sampling grid can introduce tiny interpolation
                # differences.  This branch makes the first teacher-anchored
                # checkpoint exactly, bit-for-bit, equal to the strong prior.
                ...
                residuals = (
                    ...
                    else [residual * residual_scale for residual in raw_residuals]
                )
                flows = [
                    compose_backward_flows(prior_flow, residual)
                    for residual in residuals
                ]
                final_valid = flow_valid_mask(flows[-1], warped.shape[-2:])
        final_flow = flows[-1]
        output.update(
            flows=flows,
            residuals=residuals,
            ...
            residual_application_scale=residual_scale,
            # Raw confidence is supervised/calibrated.  Effective confidence
            # is the legacy raw*availability diagnostic.  Matching and context
            # confidence report the two gates actually consumed by the refiner.
            feature_confidence=raw_confidence,
            effective_feature_confidence=effective_confidence,
            matching_feature_confidence=matching_confidence,
            context_feature_confidence=context_confidence,
...
def build_learned_geometry_prior(
    model_config: dict[str, Any],
) -> DocumentGeometryPrior:
    """Build the learned prior alone, including its exact checkpoint schema."""

    return DocumentGeometryPrior(
        base_channels=int(model_config.get("prior_base_channels", 32)),
        max_displacement_ratio=float(
            model_config.get("prior_max_displacement_ratio", 0.35)
        ),
        control_stride=int(model_config.get("prior_control_stride", 8)),
        global_pose_enabled=bool(
            model_config.get("prior_global_pose_enabled", False)
        ),
        global_pose_hidden_channels=int(
            model_config.get("prior_global_pose_hidden_channels", 256)
        ),
        global_pose_pool_size=int(
            model_config.get("prior_global_pose_pool_size", 4)
        ),
        global_pose_max_linear_delta=float(
            model_config.get("prior_global_pose_max_linear_delta", 2.50)
        ),
        global_pose_max_translation_ratio=float(
            model_config.get("prior_global_pose_max_translation_ratio", 0.50)
        ),
        global_pose_max_perspective=float(
            model_config.get("prior_global_pose_max_perspective", 0.15)
        ),
    )
...
def build_unified_rectifier(
    ...
    *,
    ...
    if prior_backend == "learned":
        prior: nn.Module = build_learned_geometry_prior(model_config)
    elif prior_backend == "torchscript":
        teacher_path = model_config.get("prior_torchscript_path")
        if not teacher_path:
            raise ValueError(
                "prior_torchscript_path is required when prior_backend=torchscript"
            )
        teacher_sha256 = model_config.get("prior_torchscript_sha256")
        if not isinstance(teacher_sha256, str) or not teacher_sha256:
            raise ValueError(
                "prior_torchscript_sha256 is required when "
        ...
        prior = TorchScriptGeometryPrior(
            ...
            autocast_dtype=str(
                model_config.get(
                    "prior_torchscript_autocast_dtype",
                    model_config.get("prior_torchscript_autocast", "float16"),
                )
            ),
        ...
        raise ValueError(
            "prior_backend must be 'learned' or 'torchscript', "
            f"got {prior_backend!r}"
        )
    ...
    return UnifiedDocumentRectifier(
        ...
        match_confidence_power=float(model_config.get("match_confidence_power", 1.0)),
        match_confidence_cap=float(model_config.get("match_confidence_cap", 1.0)),
        context_confidence_floor=float(
            model_config.get("context_confidence_floor", 0.0)
        ),
        detach_confidence_from_refiner=bool(
            model_config.get("detach_confidence_from_refiner", False)
        ),
    )
