#!/usr/bin/env bash
# 实验 F：四层融合 Layer 12 + 24 + 36 + 48（block index 11,23,35,47）
# 目的：验证完整多层融合（早层+中层+中后层）是否比三层更好
# 这是"增强版"实验，在实验 E 基础上加入早层局部特征

csv_path=/juicefs-algorithm/data/IPT/zhuochu_yang/upwarp_img_1in10_white_0511/metadata_with_flow.csv
train_id=20260515-exp_F_layer12_24_36_48

# 使用纯 DiT LoRA ckpt（无 FlowHead，只有文档矫正 LoRA，668k steps 训练更充分）
...
accelerate launch train_flow_head_v4_layer_probe.py \
  ...
  --output_path "/juicefs-algorithm/data/IPT/zhuochu_yang/DiffSynth-Studio/flow_head_v4_ckpts/$train_id" \
