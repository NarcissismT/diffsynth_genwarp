#!/bin/bash
slurm_root=/juicefs-algorithm/data/IPT/haonan_wu/Slurm
...
for ((divide_index=0; divide_index<total_divide; divide_index++)); do
  ...
  --output_dir "$slurm_root/result$ckpt.visualization/$test_set$short_side_pixel_postfix$img_size_postfix$enable_dit_fp8_computation_postfix" \
