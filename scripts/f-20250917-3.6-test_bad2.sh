...
ckpt="/juicefs-algorithm/data/IPT/yuang_feng/DiffSynth-Studio/result/20250915-1_1in10_unwarp/step-104000.safetensors"
...
# 运行 Python 脚本，传递 --crop 参数
 python /juicefs-algorithm/data/IPT/yuang_feng/DiffSynth-Studio/qwen_image_batch_full_lora.py \
  --input_dir "$test_set" \
  --lora_path $ckpt \
  --output_dir "$ckpt.visualization/0917-3.6_bad2_1024_$resize_mode" \
  --prompt "Transform the document image to a flat, front-facing view by correcting any skew, curl, or distortion. Preserve all original visual content, layout, and details without removing or altering any part of the image." \
  --batch_size 1 \
