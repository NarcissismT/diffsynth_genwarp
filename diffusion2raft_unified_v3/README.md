...
和 refiner。修订配置默认前 8 个 epoch 将 prior 学习率设为 0；随后才以 `5e-6`
联合微调，先让残差与对应特征稳定，避免 refiner 和 prior 过早共同漂移。

已经跑到旧 unified epoch 3 时，不需要回到 Stage A，也不要切换为 Q/K；直接续训：

```bash
torchrun --nproc_per_node=8 -m diffusion2raft.train \
  --config configs/unified.yaml \
  --stage unified \
  --resume runs/d2r/unified/latest.pt \
  --epochs 20
```

模型参数键没有改变，Adam moments 也会恢复；脚本会按新配置重新设置 LR，并把 prior
重新冻结到 epoch 8。注意 `--epochs` 是总 epoch 数，不是“再跑 20 个”。
...
相关体输入已经 L2 归一化。为避免旧 epoch-3 refiner 的输入分布突变，修订配置在
epoch 4 先以 `sqrt(96)=9.798` temperature 精确复现旧 scale，再在 epoch 5～6
log-ramp 到 cosine scale 1.0；用于对应监督的 softmax 独立使用 0.1 temperature。
