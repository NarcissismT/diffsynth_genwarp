...
==================================================================
推理流程与 V4 完全一致，仅模型类换成 FlowHeadV4_1（utils/flow_head_v4_1_grad.py）。

两阶段推理：
  Stage 1 - Qwen 扩散完整推理（50步），同时在 target_layers 提取 Q/K 特征
            → corrected_low（几何参考图）+ q_features, k_features
  Stage 2 - FlowHeadV4_1(corrected_low, warped, q_feats, k_feats) → flow_low
...
输出文件（每张输入图片生成 4 个）：
  ...
  c_*.jpg  FlowHead V4.1 warp 高清结果（最终输出，文字来自原图像素）
  d_*.jpg  对比图（原始 | 扩散 | V4.1 warp）

可选诊断：
  --save_flow_vis       保存 6 张 flow 诊断图（dx/dy/mag/quiver/jacobian/fold）
  --use_gt_corrected_dir <DIR>  用 GT 替代 corrected_low（domain gap 诊断）

用法：
  bash scripts/flow_v4_1_sample.sh
"""
...
from utils.flow_head_v4_1_grad import FlowHeadV4_1
...
def main():
    ...
    parser.add_argument("--flow_iters",  type=int, default=4,
                        help="FlowHead 迭代次数。训练用 4，推理也用 4 保证 over-refinement"
                             "不会把可用 flow 推到震荡水波纹（diagnose.md Step 2）")
    # LoRA
