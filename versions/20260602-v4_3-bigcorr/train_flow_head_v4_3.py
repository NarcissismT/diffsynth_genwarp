#!/usr/bin/env python3
"""
FlowHead V4.3 (Path B + 大搜索范围) 训练脚本
==============================================
在 V4.2 (path B 双图跨段 Q/K) 基础上，针对【过拟合诊断】暴露的瓶颈做两处改动：

过拟合诊断结论（18 个固定分层样本，训 3000 步）：
  low (位移<22px) : <5px = 97.4%  ← 几乎完美过拟合
  mid (22-32px)   : <5px = 96.3%  ← 几乎完美过拟合
  high (>32px)    : <5px = 53.0%  ← 卡住
  → 不是容量瓶颈（低中位移能完美拟合），是【大位移搜索范围不足】：
    V4.2 CorrBlock radius=4 @H/8 等效原图 ±32px，high 样本位移 >32px 超出搜索范围。

V4.3 改动：
  1. CorrBlock radius 4 → 8（等效搜索范围 ±32px → ±64px），见 utils/flow_head_v4_3.py
  2. iters 4 → 12（RAFT 标准，让 GRU 多步把大位移挪进搜索范围）

继承 V4.2 的全部正确逻辑（双图拼接、跨段 Q/K、timestep/1000、--k_source 开关）。

注意：radius 改变 → ConvGRU input_dim 改变，V4.2 ckpt 不能加载，需从头训练。
"""
...
from utils.flow_head_v4_3 import (
    FlowHeadV4_3,
...
class QwenImageFlowV4_3TrainingModule(DiffusionTrainingModule):
    """
    ...
    def __init__(
        ...
        # ---- FlowHead V4.3（radius=8 大搜索范围 + iters=12，默认值在模型 __init__）----
        self.flow_head = FlowHeadV4_3(
            iters=12,           # V4.3：RAFT 标准 12 次迭代
            radius=8,           # V4.3：搜索范围 ±8(@H/8, 等效原图±64px)
    ...
    def forward(self, data, inputs=None):
        ...
        predictions = self.flow_head(corrected_t, warped_t,
                                     q_features, k_features, iters=12)
