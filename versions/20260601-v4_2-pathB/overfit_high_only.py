...
from train_flow_head_v4_2_pathB import (
    QwenImageFlowV4_2TrainingModule,
    parse_layer_list,
)
...
def load_fixed_samples(csv_path: str, base_path: str, max_pixels: int, only_layer: str = None):
    """加载固定分层样本，返回 list of data dict（image/edit_image=PIL, flow_gt=tensor）。
    only_layer: 若指定（如 'high'），只保留该层样本。"""
    rows = list(csv.DictReader(open(csv_path)))
    samples = []
    for r in rows:
        if only_layer and r.get("_layer", "") != only_layer:
            continue
...
def evaluate(model, samples, device, iters):
    ...
    for s in samples:
        ...
        by_layer.setdefault(layer, []).append({
            ...
            "sample_zero": s.get("_zero_epe", 0.0),  # 该样本原始 zero_epe（用于 per-sample 排序）
        })
    model.train()
    return by_layer
...
def main():
    ...
    parser.add_argument("--num_steps", type=int, default=8000,
                        help="high-only 长训：默认 8000 步（少样本，要训到收敛判断天花板）")
    parser.add_argument("--eval_every", type=int, default=200)
    parser.add_argument("--train_iters", type=int, default=4)
    parser.add_argument("--eval_iters", type=int, default=4)
    parser.add_argument("--only_layer", type=str, default="high",
                        help="只用某层样本（high-only 诊断默认 high）")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    target_layers = parse_layer_list(args.dit_target_layers)
    print(f"=== V4.2 high-only 长训过拟合诊断 ===")
    print(f"目标层: {target_layers}, k_source: {args.k_source}, lr: {args.learning_rate}")
    print(f"训练 iters: {args.train_iters}, 评估 iters: {args.eval_iters}, only_layer: {args.only_layer}")
    print(f"判据：6 个 high 样本长训 {args.num_steps} 步——")
    print(f"  能爬到 80%+ <5px → high 只是收敛慢，V4.2 多训即可")
    print(f"  死卡 ~53% 不动     → 架构天花板（分辨率/特征判别力），需 convex upsample/多尺度")

    # 加载固定样本（只 high 层）
    samples = load_fixed_samples(args.samples_csv, args.dataset_base_path,
                                 args.max_pixels, only_layer=args.only_layer)
    print(f"加载 {len(samples)} 个 {args.only_layer} 层样本")
    ...
    # per-sample 明细：区分"全部 high 卡住"还是"个别极端样本拖累"
    print("\n各 high 样本单独 <5px（按 zero_epe 排序）:")
    rs = sorted(final.get("high", []), key=lambda r: r["sample_zero"])
    for r in rs:
        print(f"  zero_epe={r['sample_zero']:7.2f}  EPE={r['epe']:6.2f}  "
              f"<3px={r['p3']*100:5.1f}%  <5px={r['p5']*100:5.1f}%")
    # 排除极端离群样本（zero_epe>100）后的均值
    normal = [r for r in rs if r["sample_zero"] <= 100]
    if normal and len(normal) < len(rs):
        np5 = np.mean([r["p5"] for r in normal]) * 100
        print(f"  [排除 zero_epe>100 的极端样本后] high <5px = {np5:.1f}%（{len(normal)} 个样本）")

    overall_p5 = np.mean([r["p5"] for rs2 in final.values() for r in rs2]) * 100
    print(f"\nhigh 层 <5px = {overall_p5:.1f}%（全部 {len(samples)} 个样本，长训 {args.num_steps} 步）")
    if overall_p5 > 80:
        print("→ high 能爬到 80%+：只是收敛慢，high 本身能学。"
              "结论：V4.2 架构够用，把全量训练跑到收敛即可，不必改架构。")
    elif overall_p5 < 60:
        print("→ high 死卡 <60%：确认是架构天花板（H/8 分辨率 / 特征判别力）。"
              "结论：需要 convex upsampling 或更高分辨率输出 / 多尺度，不是搜索范围问题。")
    else:
        print("→ 60-80% 中间：部分 high 样本能学、部分卡住。"
              "看是哪几个样本（极端 168px 那个？）拖累，可能是少数极端样本的问题。")
