...
def process_images(input_dir, output_dir, dit_path, lora_path, prompt, batch_size=1, prompt_dict=None, short_side_pixel=2048, img_size=1024, enable_dit_fp8_computation=False, total_divide=1, divide_index=0, infer_steps=50, resize_mode = "stretch"):
    """批量处理图片并保持原始宽高"""
        ...
        for img_path in batch_files:
            ...
            try:
                ...
                if resize_mode == "crop":
                    ...
                    # img_cropped = img.resize((short_side_pixel, short_side_pixel), Image.LANCZOS)
                    new_width, new_height = img_size, img_size
                    print(f"缩放至: {img_size}x{img_size}")
                ...
                # 创建对比图
                # 创建一个新图像，宽度为两倍 new_width，高度为 new_height
                compare_img = Image.new('RGB', (2 * new_width, new_height))

                # 将两张图像粘贴到新图像上
                compare_img.paste(img_cropped, (0, 0))  # 左边
                compare_img.paste(enhanced_img, (new_width, 0))  # 右边
...
if __name__ == "__main__":
    ...
    # 批量处理图片
    process_images(args.input_dir, args.output_dir, args.dit_path, args.lora_path, args.prompt, args.batch_size, args.prompt_dict, args.short_side_pixel, args.img_size, args.enable_dit_fp8_computation, args.total_divide, args.divide_index,args.infer_steps, args.resize_mode)
