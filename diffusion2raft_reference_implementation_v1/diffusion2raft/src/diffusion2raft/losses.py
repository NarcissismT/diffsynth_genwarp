...
def reconstruction_losses(
    ...
    rectified = backward_warp(warped, flow, padding_mode="border")
    photo = _masked_mean(robust_l1(rectified - target), valid)
    ...
    def forward(
        ...
        if outputs["residuals"]:
            residual = _masked_mean(
                torch.linalg.vector_norm(outputs["residuals"][-1], dim=1, keepdim=True),
                valid,
            )
        else:
        ...
        weighted = {
            ...
            "prior_flow": float(self.config.get("prior_flow", 0.5)) * prior_loss,
