...
建议依次做四组，四组都有对应的 config / 开关：

1. warped-only prior —— `--stage prior`；
2. 直接 `RAFT(guide, warped)` —— `configs/ablation_direct.yaml`（`model.residual_mode: direct`）；
3. prior + bounded residual RAFT —— `configs/base.yaml --stage joint`，`guide_artifact_prob` / `guide_dropout_prob` 设为 0；
4. 第 3 组 + guide artifact/dropout + anti-fold —— `configs/base.yaml` 默认值。

```bash
# 第 2 组：direct RAFT baseline
d2r-train --config configs/ablation_direct.yaml --stage joint --epochs 20
```

`model.residual_mode` 取值：

- `compose`（默认）：prior 预矫正 → RAFT 预测受限残差 → 几何复合 `R(x)+B(x+R(x))`；
- `direct`：RAFT 直接从 `guide->warped` 输出完整 flow，无预矫正、无残差 bound、无 guide dropout，仅作 baseline。

主要指标不要只用 PSNR/SSIM，应至少包含：flow EPE、Jacobian fold rate、OCR CER、文字边缘重投影误差、直线弯曲度和无效采样比例。`d2r-eval` 直接输出除 OCR CER 外的全部纯几何指标：
...
d2r-eval \
  ...
  --output-json runs/d2r/joint/val_metrics.json
```

输出键：`epe`、`fold_rate`、`near_fold_rate`、`jacobian_p01`、`invalid_ratio`、
`line_bending`（直线弯曲度）、`text_edge_epe`（文字边缘重投影误差）。OCR CER 需
额外的 OCR 引擎和 manifest 里的 GT 文本，用 `--ocr` 显式开启；缺依赖或缺 GT 文本时
自动跳过而不报错。训练 / 验证阶段也会实时打印 `epe / fold_rate / invalid_ratio /
line_bending`，推理的 `*_metadata.json` 里同样带 `invalid_ratio` 和 `line_bending`。
...
- 本仓库提供完整训练、推理、评测、Qwen guide 缓存、坐标变换、损失和消融开关实现，但不包含 Qwen、RAFT 权重或你的 GT flow 数据。
- 纯 NumPy 几何验证（`scripts/validate_geometry_numpy.py` + `tests/`）通过；1-batch CUDA smoke test 已在 `torch 2.10 + cu128` 上跑通：prior → joint（compose）→ joint（direct baseline）→ 推理 → `d2r-eval` 全流程正常，loss 下降、fold_rate 低。换到你自己的训练机后建议再跑一次确认环境。
- OCR CER 是唯一可选依赖项（`easyocr`），默认不参与训练 / 评测；只在 `--ocr` 且 manifest 带 GT 文本时启用。
- DA-Flow V3 的 Q/K 提取、FlowDiffuser / OFM 后端仍按 README 判断暂不实现，留待完成前置实验后再接入 bounded-residual backend。
