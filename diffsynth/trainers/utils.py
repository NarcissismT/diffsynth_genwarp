...
class ImageDataset(torch.utils.data.Dataset):
    ...
    #如果启用动态分辨率，则根据最大像素数一起缩放长宽 并保证使其能够整除一定值
    #接下来使用该目标分辨率对原图进行resize以及crop
    def get_height_width(self, image):
        if self.dynamic_resolution:
            ...
            #将 height 和 width 向下取整为 height_division_factor 和 width_division_factor 的倍数
...
def launch_training_task(
    ...
    #尝试添加batchsize
    # print("✅ Loaded modified utils.py")
    # dataloader = torch.utils.data.DataLoader(dataset, batch_size = batch_size, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
