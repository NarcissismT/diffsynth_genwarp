...
from ..utils.flow_guidance import backward_warp_latents
...
class QwenImagePipeline(BasePipeline):
    ...
    def __call__(
        ...
        # Optional latent-space backward-flow guidance.  The flow is sampled
        # from the original RGB canvas and only applied during late denoising.
        flow_guidance: torch.Tensor = None,
        flow_guidance_scale: float = 0.0,
        flow_guidance_start: float = 0.5,
        flow_guidance_end: float = 1.0,
        flow_guidance_source_size: tuple[int, int] = None,
        flow_guidance_detach_anchor: bool = True,
        flow_guidance_noisy_anchor: bool = True,
        # FP8
        ...
        inputs_shared = {
            ...
            "flow_guidance_scale": float(flow_guidance_scale),
            "flow_guidance_start": float(flow_guidance_start),
            "flow_guidance_end": float(flow_guidance_end),
            "flow_guidance_source_size": flow_guidance_source_size,
            "flow_guidance_detach_anchor": bool(flow_guidance_detach_anchor),
            "flow_guidance_noisy_anchor": bool(flow_guidance_noisy_anchor),
        }
        ...
        has_flow_guidance = flow_guidance is not None and float(flow_guidance_scale) > 0.0
        flow_anchor_latents = None
        if has_flow_guidance:
            if flow_guidance.ndim not in (3, 4) or flow_guidance.shape[-3] != 2:
                raise ValueError(
                    "flow_guidance must have shape [2,H,W] or [B,2,H,W], "
                    f"got {tuple(flow_guidance.shape)}"
                )
            if not 0.0 <= float(flow_guidance_start) <= float(flow_guidance_end) <= 1.0:
                raise ValueError(
                    "flow_guidance window must satisfy 0 <= start <= end <= 1"
                )
            flow_guidance = flow_guidance.to(device=self.device, dtype=torch.float32)
            # ``input_latents`` is the unwarped source conditioning image in
            # Qwen Edit.  It is the only faithful carrier of small glyphs.
            # ``edit_latents`` is supported for callers using the separate
            # edit-image input; both have the same latent canvas convention.
            flow_anchor_latents = inputs_shared.get("input_latents")
            if flow_anchor_latents is None:
                flow_anchor_latents = inputs_shared.get("edit_latents")
            if flow_anchor_latents is None:
                raise ValueError(
                    "flow guidance requires input_latents or edit_latents; "
                    "pass input_image/edit_image to the pipeline"
                )
            flow_anchor_latents = flow_anchor_latents.to(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Late-step flow anchoring: preserve source pixels by moving the
            # current latent canvas according to the requested backward flow,
            # then blend it softly into the scheduler result.  This is a
            # post-update constraint, so existing model/checkpoint interfaces
            # remain unchanged and scale=0 is exactly the legacy path.
            if has_flow_guidance:
                progress = (progress_id + 1) / max(len(self.scheduler.timesteps), 1)
                if float(flow_guidance_start) <= progress <= float(flow_guidance_end):
                    anchor_source = backward_warp_latents(
                        flow_anchor_latents,
                        flow_guidance,
                        source_size=flow_guidance_source_size,
                    )
                    if flow_guidance_noisy_anchor:
                        # Match the current flow-matching noise level.  The
                        # same noise realization keeps glyph identity while
                        # preventing a clean latent from overwriting global
                        # denoising semantics.
                        sigma = self.scheduler.sigmas[progress_id].to(
                            device=self.device, dtype=flow_anchor_latents.dtype
                        )
                        anchor_noise = inputs_shared["noise"].to(
                            device=self.device, dtype=flow_anchor_latents.dtype
                        )
                        anchor = (1.0 - sigma) * anchor_source + sigma * anchor_noise
                    else:
                        anchor = anchor_source
                    if flow_guidance_detach_anchor:
                        anchor = anchor.detach()
                    strength = float(flow_guidance_scale)
                    # Avoid an unintuitive >100% blend while still allowing
                    # callers to pass values above one as a hard clamp.
                    strength = max(0.0, min(1.0, strength))
                    inputs_shared["latents"] = (
                        (1.0 - strength) * inputs_shared["latents"] + strength * anchor
                    )
...
def model_fn_qwen_image(
    ...
    **kwargs
    ...
    image = dit.img_in(image) # torch.Size([1, 8056, 64])
