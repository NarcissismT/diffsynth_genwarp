    ...
    for i in tqdm(range(0, len(image_files), batch_size), total=total_batches, desc="处理图片批次"):
        ...
        for img_path in batch_files:
            ...
            try:
                ...
                # 如果尺寸曾经调整过，恢复到原始尺寸
                enhanced_img = enhanced_img.resize((orig_width, orig_height), Image.Resampling.LANCZOS)
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='使用FLUX模型批量处理图片')
    parser.add_argument('--dit_path', type=str, required=True, help='LoRA模型路径')
    ...
    parser.add_argument('--prompt', type=str, default='Enhance this image to high definition, remove reflections, and balance light and shadow.', help='处理图片使用的提示词')
